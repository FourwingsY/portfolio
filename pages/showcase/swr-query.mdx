...
export const meta = {
  ...
  written: "2022-11-05",
  keywords: ["react-query", "swr", "query", "react"],
}
...
요약하자면 redux에서 store를 쓰듯이, API 호출을 줄이기 위해 _API를 통해 불러온 데이터를 한 번만 불러와 저장해두고 반복해서 쓰려고_ 할 때
...
A: 그 훅은 Post/Put/Patch/Delete 같은 요청을 useQuery처럼 다루기 위한 훅일 뿐, 받아왔던 데이터의 Revalidating과는 다른 관점의 이야기이다. 이 섹션에서는 이야기하지 않는다. **SWR 2.0에서는 `useSWRMutation()` 훅이 추가되었다**
...
SWR 2.0이 추가되면서 공식 DevTools 지원이 추가되었다. 다만 React Query는 stale 상태를 모니터링해야 하는 필요가 있어 DevTools가 필요했다면, SWR은 그런 필요가 거의 없고 단지 "어느 시점에 갱신된 데이터를 캐싱중인지"를 확인할 수 있는 도구의 역할을 하고 있다.
