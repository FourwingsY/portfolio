...
예를 들어, dedupingInterval과 staleTime을 둘 다 10초로 설정하고, 10초 내에 페이지 이동을 A -> B -> A 로 실행할 경우, SWR은 A 페이지를 캐싱된 데이터로 렌더링 한 뒤, 새로운 데이터를 불러오지만, React Query는 처음 A 페이지를 그릴 때 받아왔던 데이터의 유효기간이 남아있으므로 추가적인 요청을 하지 않는다.
...
Q: React Query에는 useMutation 이 있다.

A: 그 훅은 Post/Put/Patch/Delete 같은 요청을 쿼리처럼 다루기 위한 훅일 뿐, 데이터의 Revalidating과는 다른 관점의 이야기이다. 이 섹션에서는 이야기하지 않는다.
...
리얼클래스를 개발하는 관점에서 상상해서 비교해보자. 리얼클래스의 데이터들은 대게 "공개 데이터"와 "개인 학습 데이터"로 구분된다. 공개 데이터는 거의 변하지 않으므로 React Query의 캐싱 옵션이 꽤 쓸만할 것 같다. 하지만 SWR에서도 `revalidateIfStale: false` 옵션을 주면, 최초 1회만 불러오는 방식으로 처리할 수 있다. 그렇다면 개인 학습 데이터는 어떨까? 사실 유저가 학습 중인 화면에서는, 생각보다 학습 데이터를 업데이트 할 일이 별로 없다. 각 단계가 끝날 때마다 해당 단계의 완료 표시를 해 줘야 하긴 하지만, 각 단계가 끝날 때 어차피 서버에 기록을 해 줘야 하기 때문에 SWR나 React Query의 캐시 데이터를 업데이트 해 주는 정도는(위의 revalidation) 동일하게 적용할 수 있다. 학습 후 다른 페이지로 돌아갔을 때 잘 업데이트 되는 것이 중요하다. 음... 이 부분은 SWR의 기본 옵션을 사용하는 편이 더 편할 것 같다는 느낌이 들지만 React Query의 기본 staleTime 도 0 이므로 사실 별 차이가 없을 것 같다. 결국 옵션을 잘 쓰면 어떤 라이브러리를 사용하던 큰 차이가 없다는 결론이 나오는 것 같다.

이 외의 관점에서 React Query의 가장 큰 매력 포인트는 공식 DevTools를 제공한다는 점이다. 비공식적인 SWR Devtools도 있긴 하지만, 공식적인 방법을 제공해주면 더 좋았을 것 같다.

결국 내릴 수 있는 결론은: "직접, 오래 써봐야 알 것 같다." 